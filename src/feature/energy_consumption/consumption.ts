...
export class Consumption implements IConsumption {
  ...
  getConsumptionForLastDays = async (n: number): Promise<LastNDaysConsumption> => {
    const timeNowMs = Date.now();
    const dayLenInMs = 1000 * 60 * 60 * 24;
    const canonicalTimeNow = new Date(timeNowMs + dayLenInMs).setHours(0, 0, 0, 0)
  ...
  getConsumptionPerTransaction = async (hash: string): Promise<BlockWithConsumption> => {
    ...
    const transactions: TransactionWithConsumption[] = blockInfo.data.tx.map((t) => {
